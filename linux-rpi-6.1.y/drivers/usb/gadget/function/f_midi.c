...
static int f_midi_register_card(struct f_midi *midi)
{
	...
	char shortn[] = "f_midi";
	char longn[] = "MIDI Gadget";

	if (*midi->shortname == 0) {
		strcpy(card->shortname, shortn);
	} else {
		strcpy(card->shortname, midi->shortname);
	}
	if (*midi->longname == 0) {
		/*snprintf(card->longname, sizeof(longn), longn);
		snprintf(card->driver, sizeof(longn), longn);*/
		strcpy(card->driver, longn);
		strcpy(osLongname, longn);
	} else {
		strcpy(card->longname, midi->longname);
		strcpy(card->driver, midi->longname);
		strcpy(osLongname, midi->longname);
	}
...
static ssize_t f_midi_opts_shortname_show(struct config_item *item, char *page)
{
	struct f_midi_opts *opts = to_f_midi_opts(item);
	int result;
	...
	mutex_lock(&opts->lock);
	if (opts->shortname) {
		result = strlcpy(page, opts->shortname, PAGE_SIZE);
	} else {
		page[0] = 0;
		result = 0;
	}
...
static ssize_t f_midi_opts_shortname_store(struct config_item *item,
				    const char *page, size_t len)
{
	struct f_midi_opts *opts = to_f_midi_opts(item);
	int ret;
	...
	if (opts->shortname_allocated)
		kfree(opts->shortname);
	opts->shortname = c;
	opts->shortname_allocated = true;
	ret = len;
end:
	mutex_unlock(&opts->lock);
	return ret;
...
CONFIGFS_ATTR(f_midi_opts_, shortname);

static ssize_t f_midi_opts_longname_show(struct config_item *item, char *page)
{
	struct f_midi_opts *opts = to_f_midi_opts(item);
	int result;
	...
	mutex_lock(&opts->lock);
	if (opts->longname) {
		result = strlcpy(page, opts->longname, PAGE_SIZE);
	} else {
		page[0] = 0;
		result = 0;
	}
...
static ssize_t f_midi_opts_longname_store(struct config_item *item,
				    const char *page, size_t len)
{
	struct f_midi_opts *opts = to_f_midi_opts(item);
	int ret;
	...
	if (opts->longname_allocated)
		kfree(opts->longname);
	opts->longname = c;
	opts->longname_allocated = true;
	ret = len;
end:
	mutex_unlock(&opts->lock);
	return ret;
...
CONFIGFS_ATTR(f_midi_opts_, longname);
...
static struct configfs_attribute *midi_attrs[] = {
	...
	&f_midi_opts_attr_id,
	&f_midi_opts_attr_shortname,
	&f_midi_opts_attr_longname,
	NULL,
};
...
static struct usb_function_instance *f_midi_alloc_inst(void)
{
	...
	opts->shortname = "f_midi";
	opts->longname = "MIDI Gadget";
	opts->buflen = 512;
	opts->qlen = 32;
	opts->in_ports = 1;
	opts->out_ports = 1;
...
static struct usb_function *f_midi_alloc(struct usb_function_instance *fi)
{
	...
	midi->shortname = kstrdup(opts->shortname, GFP_KERNEL);
	if (opts->shortname && !midi->shortname) {
		status = -ENOMEM;
		goto midi_free;
	}
	midi->longname = kstrdup(opts->longname, GFP_KERNEL);
	if (opts->longname && !midi->longname) {
		status = -ENOMEM;
		goto midi_free;
	}
