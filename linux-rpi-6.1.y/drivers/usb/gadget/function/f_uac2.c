...
afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
{
	...
	ret = g_audio_setup(agdev, uac2_opts->local_dev_pcm_name, uac2_opts->local_dev_name);
	if (ret)
		goto err_free_descs;
...
UAC2_ATTRIBUTE_STRING(local_dev_name);
UAC2_ATTRIBUTE_STRING(local_dev_pcm_name);
...
static struct configfs_attribute *f_uac2_attrs[] = {
	...
	&f_uac2_opts_attr_local_dev_name,
	&f_uac2_opts_attr_local_dev_pcm_name,
...
static struct usb_function_instance *afunc_alloc_inst(void)
{
	...
	snprintf(opts->local_dev_name, sizeof(opts->local_dev_name), "UAC2_Gadget");
	snprintf(opts->local_dev_pcm_name, sizeof(opts->local_dev_pcm_name), "UAC2 PCM");
